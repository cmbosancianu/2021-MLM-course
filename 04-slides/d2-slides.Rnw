...
\begin{frame}[fragile]
  ...
  <<r show-random-slopes, eval=generateFigs, echo=TRUE, size = "footnotesize">>=
...
ISSPsub <- rbind(ISSPsub, ISSPin, ISSPde)
rm(ISSPin, ISSPde)
...
ggplot(finplotDF %>% arrange(-estimate) %>%
  ...
  geom_hline(yintercept = 0, size = 1.25, linetype = "dashed") +
  annotate("label", x = "VE", y = 0.50,
           label = "Red line denotes education slope from complete pooling model") +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 14),
        ...
        legend.position = "top")
invisible(dev.off())
rm(finplotDF, cpoolBeta, model2, ISSPsub)
@
...
texreg(list(model1, model2, model3),
       ...
       custom.coef.names = c("(Intercept)","Female","Education","Income: 2nd quartile",
                             "Income: 3rd quartile", "Income: 4th quartile",
                             "Urban residence", "Perceptions of corruption",
...
model5 <- lmer(poleff ~ 1 + femaleCWC + educCWC + inc2CWC + inc3CWC + inc4CWC +
                 urbanCWC + cpiCGM + educCWC*cpiCGM + giniCGM + educCWC*giniCGM +
                 (1 + educCWC| cnt),
               data = ISSPsub)
...
graph1 +
  ...
  geom_hline(yintercept = 0, linetype = "dashed",
             size = 1.25, color = rgb(255, 0, 90, maxColorValue = 255)) +
...
<<r data-manage-1, eval=generateFigs, echo=TRUE>>=
