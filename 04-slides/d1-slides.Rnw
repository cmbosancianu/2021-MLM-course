...
library(pacman)
p_load(tidyverse, scales, texreg, nlme, broom, arm, ggthemes,
       kableExtra)
...
graph1 <- ggplot(mlDF %>% arrange(pse) %>%
                   mutate(culture = factor(culture,
                                           levels = culture[group == "Adults"])),
                 aes(x = culture,
                     y = pse,
                     fill = group)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_clean() +
  scale_fill_Pres(name = "Population") +
...
df_sub %>%
  ...
  annotate("label", x = "VE", y = 0.07,
           label = "Red line is slope for education from complete pooling model") + # nolint
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 14),
        axis.text.y = element_text(size = 14),
        axis.title.y = element_text(size = 16))
invisible(dev.off())
